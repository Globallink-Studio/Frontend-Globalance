...
import { dashboardMock } from '../../src/data/mocks'
...
describe('dashboard API — modo mock (desarrollo local)', () => {
  ...
  test('getDashboardMetrics devuelve las métricas mock', async () => {
    const metrics = await getDashboardMetrics()
    expect(metrics).toEqual(dashboardMock.metrics)
  })

  test('getDashboardChart devuelve el gráfico mock', async () => {
    const chart = await getDashboardChart()
    expect(chart).toEqual(dashboardMock.chart)
  })
})
